    ...
    <nav className="bg-gray-900 text-white shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="text-xl font-bold">
          <Link href="/">
            <span className="hover:text-blue-400 transition cursor-pointer">Nexa Cars</span>
          </Link>
        </div>
      ...
      <div
        id="sidebar"
        className="fixed top-0 right-0 w-full h-full bg-white z-50 transform translate-x-full flex flex-col items-center justify-center space-y-8"
      >
        <button
...
const NavItem = ({ href, children, isSidebar }: { href: string; children: React.ReactNode; isSidebar?: boolean }) => {
  return (
    <Link href={href}>
      <span
        className={`relative group text-lg transition cursor-pointer ${
          isSidebar ? "text-gray-900 hover:text-blue-400" : "hover:text-blue-400"
        }`}
      >
        {children}
        {!isSidebar && (
